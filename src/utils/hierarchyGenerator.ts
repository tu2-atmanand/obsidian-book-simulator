		...
		if (folder.children && folder.children.length > 0) {
			...
			for (const file of files) {
				if (file.name && file.name.endsWith('.md')) {
					...
					try {
						const fileObj = app.vault.getAbstractFileByPath(file.path);
						if (fileObj && fileObj instanceof TFile) {
							const content = await app.vault.read(fileObj);
							const processedContent = processNoteContent(content, headerLevel + 2);
			...
			for (const subfolder of folders) {
				const subHierarchy = await generateHierarchyMarkdown(app, subfolder, headerLevel + 1);
